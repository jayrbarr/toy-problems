...
Range.prototype.size = function () {
  let size = null;
  if (this._step) {
    size = Math.floor(Math.abs(this._end - this._start)/Math.abs(this._step)) + 1;
  } else if (typeof this._end !== 'undefined') {
    size = Math.floor(Math.abs(this._end - this._start)) + 1;
  } else if (typeof this._start !== 'undefined') {
    size = 1;
  }
  return size;
};
...
Range.prototype.includes = function (val) {
  if (typeof this._end !== 'undefined') {
    if (this._start < this._end) {
      if (this._step) {
        return val >= this._start && val <= this._end && (val - this._start) % this._step === 0;
      } else {
        return val >= this._start && val <= this._end;
      }
    } else {
      if (this._step) {
        return val <= this._start && val >= this._end && (val - this._start) % this._step === 0;
