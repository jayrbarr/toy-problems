...
<<<<<<< HEAD
  let numbers = Array.from({ length: end - 1 }, (_, i) => i+2);
  let returnArray = [];
  let nextPrime = numbers.shift();
  while (nextPrime) {
    if (nextPrime >= start) returnArray.push(nextPrime);
    numbers = numbers.filter(num => num%nextPrime);
    nextPrime = numbers.shift()
  }
  return returnArray;
};

// console.log(primeTester(17));
// console.log(primeTester(169));

// console.log(primeSieve(3, 21));

=======

  var current = 2; var primes = range(0, end); // eslint-disable-line
