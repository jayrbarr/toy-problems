...
var mergeSort = function(array) {
  ...
  // Split array in half.
  let left = array.slice(0, len/2);
  let right = array.slice(len/2);

  // Sort sides recursively.
  let sortedLeft = (arraySorted(left)) ? left : mergeSort(left);
  let sortedRight = (arraySorted(right)) ? right : mergeSort(right);
    ...
    if (sortedLeft[0]) {
      if (sortedRight[0]) {
        if (sortedLeft[0] < sortedRight[0]) {
          retArr.push(sortedLeft.shift());
        } else retArr.push(sortedRight.shift());
      } else retArr.push(sortedLeft.shift());
    } else retArr.push(sortedRight.shift());
  }
  return retArr;
};

var arraySorted = function(array) {
  let prev = array[0];
