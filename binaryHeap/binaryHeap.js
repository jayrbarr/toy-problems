...
BinaryHeap.prototype._getLesserChildIndex = function (parentIndex) {
  const childIndices = [parentIndex * 2 + 1, parentIndex * 2 + 2].filter(index => index < this._heap.length);
  return this._compare(this._heap[childIndices[0]], this._heap[childIndices[1]]) ? childIndices[0] : childIndices[1];
}

BinaryHeap.prototype._swap = function (child, parent) {
  const temp = this._heap[child];
...
BinaryHeap.prototype.insert = function (value) {
  ...
  while (parentIndex !== -1 && this._compare(this._heap[index], this._heap[parentIndex])) {
